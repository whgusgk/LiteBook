...
<head>
	...
	<title>UserCardUpdateForm.jsp</title>
   ...
   <script>
	    ...
	    $(function()
	    {
			...
			kakao.maps.event.addListener(marker, "click", function()
		   {
			...
			// 이전 페이지로부터 넘어온 데이터 값 세팅 (기존 데이터)
			var hour = ${fn:substring(card.card_time, 0, 2)};
			var minute = ${fn:substring(card.card_time, 2, 4)};
			
			if (minute="0")
				minute = "00"
			
			$("#hour").val(hour).prop("selected", true);
			$("#minute").val(minute).prop("selected", true);
			
			
			// alert(${card.card_time});
			// alert(minute);
			...
			$(".payAdd").click(function()
			{
				...
				divNumEl.className = "pay_num";
				divNameEl.className = "pay_name";
				divKindEl.className = "pay_kind";
				divAmountEl.className = "pay_amount";
				divDeleteEl.className = "pay_delete";
				
				// div 영역 안에 들어갈 구문 구성
				var inputNum = "<input type='hidden' class='payNum' name='payNum' value='0'>";
				var inputName = "<input type='text' class='payName' name='payName' placeholder='지출명'>";
				var select = "<select name='payKind' >";
					select += "<option value='1'>카드</option>";
					select += "<option value='2'>현금</option>";
					select += "<option value='3'>기타</option>";
					select += "</select>";
				var inputAmount = "<input type='text' class='payAmount' name='payAmount' placeholder='지출금액' value='0'>";
				// var button = "<button type='button' class='btn btn-default control payDeleteBtn' onclick='deletePayBtn()'>삭제</button>";
				var button = "<button type='button' class='btn btn-default control payDeleteBtn'>삭제</button>";
				...
				divDetailEl.appendChild(divNumEl);
			 ...
			 $("#confirmBtn").click(function()
       		{
				...
				{
					$("#cardForm").attr("action", "usercardpayupdate.action");
	       			$("#cardForm").submit();
				}
       			else
       			{
					$("#cardForm").attr("action", "usercardupdate.action");
	       			$("#cardForm").submit();
				}
	    ...
	    /* 지출내역 삭제 함수 / $(document) 영역으로 이동
	    function deletePayBtn()
		{
	    	// 삭제 버튼이 속한 영역의 인덱스 값
	    	var idx = $(".oldPayDeleteBtn").index(this);
	    	
	    	// alert($("#payNum").eq(idx).val());
...
<div class="info">
    ...
    <div class="input-group input_place">
        ...
        <div id="hidden">
        	...
        	<input type="hidden" id="day" name="day" value="${card.card_visitdate }">
        	<input type="hidden" id="card_num" name="card_num" value="${card.card_num }">
    ...
    <div class="input-group input_memo">
        ...
        <textarea rows="5" cols="111" id="memo" name="memo" class="form-control" placeholder="내용을 입력해주세요.">${card.card_comment }</textarea>
        <!-- 메모부분 수정 필요 -->
    </div>
		...
		<div class="uploadFiles">
			<div class="image" id="img1">
				...
				<c:if test="${card.card_img1 ne '' }">
					<img alt="card_img1" src="${card.card_img1 }">
				</c:if>
			</div>
			<div class="image" id="img2">
				사진2
				<c:if test="${card.card_img2 ne '' }">
					<img alt="card_img1" src="${card.card_img2 }">
				</c:if>
			</div>
			<div class="image" id="img3">
				사진3
				<c:if test="${card.card_img3 ne '' }">
					<img alt="card_img1" src="${card.card_img3 }">
				</c:if>
			</div>
		    사진1 <input type="file" name="uploadFile1" >
		    사진2 <input type="file" name="uploadFile2" >   
		    사진3 <input type="file" name="uploadFile3" >
		</div>
    </div>
		...
		<div class="pays" id="pays">
			<c:forEach var="pay" items="${pays }">
				<div class="pay_detail">
					<div class="pay_num">
						<input type="hidden" class="payNum" name="payNum" value="${pay.pay_num }">
					</div>
					<div class="pay_name">
						<input type="text" class="payName" name="payName" value="${pay.pay_name }">
					</div>
					<div class="pay_kind" >
						<select  name='payKind'>
							<option value="1" ${pay.ptype_num eq 1 ? 'selected' : '' }>카드</option>
							<option value="2" ${pay.ptype_num eq 2 ? 'selected' : '' }>현금</option>
							<option value="3" ${pay.ptype_num eq 3 ? 'selected' : '' }>기타</option>
						</select>
					</div>
					<div class="pay_amount">
						<input type="text" class="payAmount" name="payAmount" value="${pay.pay_amount }">
					</div>
					<div class="pay_delete">
						<button type='button' class='btn btn-default control payDeleteBtn'>삭제</button>
					</div>
				</div>
			</c:forEach>
		</div>
    ...
    <div class="action">
        ...
        <button type="button" class="btn btn-primary control confirmBtn" id="cancelBtn">취소</button>
    </div>
    </form>
