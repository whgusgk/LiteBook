...
public class UserMyQnAController implements Controller
{
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			int count = dao.qnaCount(sign_num);
			...
			// 페이징 문자열
			String pageUrl = "usermyqna.action";
			
			String pageList = page.getIndexList(currentPage, totalPage, pageUrl);
			mav.addObject("pageList", pageList);
			
			// 제목을 눌렀을 때 이동할 링크 구성
			String qnaUrl = "useranswer.action?pageNum=" + currentPage;
			mav.addObject("qnaUrl", qnaUrl);
			
			// 질문 리스트
			qnaList = dao.qnaList(sign_num, start, end);
			
			mav.addObject("qnaList", qnaList);
			
			// 뷰 페이지 set
			mav.setViewName("/WEB-INF/userView/UserMyQnA.jsp");
