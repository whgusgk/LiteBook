...
public class UserCardPayUpdateController implements Controller
{
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			card.setCard_num(Integer.parseInt(card_num));

			dao.cardUpdate(card);
			
			// 저장할 데이터 변수에 옮겨 담기 (지출)
			   String [] payNums = req.getParameterValues("payNum");
			   String [] payNames = req.getParameterValues("payName");
			   String [] payKinds = req.getParameterValues("payKind");
			   String [] payAmounts = req.getParameterValues("payAmount");
			
			for (int i = 0; i < payNames.length; i++)
			{
				// payNum의 값이 0이면 신규 입력이기 때문에 insert 실행
				if (payNums[i].equals("0"))
				{
					PayDTO pay = new PayDTO();
					
					pay.setCard_num(String.valueOf(card_num));
					pay.setPay_name(payNames[i]);
					pay.setPtype_num(payKinds[i]);
					pay.setPay_amount(payAmounts[i]);
					
					dao.payInsert(pay);
				}
				// 0이 아니라면 기존의 내용을 수정하는 것이기 때문에 update 실행
				else
				{
					PayDTO pay = new PayDTO();
					
					pay.setPay_num(payNums[i].trim());
					pay.setPay_name(payNames[i].trim());
					pay.setPtype_num(payKinds[i].trim());
					pay.setPay_amount(payAmounts[i].trim());
